# Script to fetch policy set definition and convert to JSON
param(
...
Write-Verbose "Starting policy set definition fetch for ID: $PolicyId"
...
try {
    ...
    # Fetch the policy
    $policy = Get-AzPolicySetDefinition -Id $PolicyId
    
    # Validate policy was retrieved
    if ($null -eq $policy) {
        Write-Error "Policy set not found or access denied for ID: $PolicyId"
        exit 1
    }
    
    Write-Verbose "Successfully retrieved policy set: $($policy.Name)"
    ...
    $policyOutput = @{
        ...
        PolicyDefinitions = $policy.PolicyDefinitions
        PolicyDefinitionGroups = $policy.PolicyDefinitionGroups
        Parameters = @{}
        Type = $policy.Type
        Id = $policy.Id
    }
    ...
    # Extract parameters
    if ($policy.Parameter) {
        Write-Verbose "Found parameters in policy set, processing..."
        $paramObject = @{}
        ...
        Write-Verbose "No parameters found in policy set"
    }
    ...
    Write-Verbose "Error getting policy set definition: $_"
